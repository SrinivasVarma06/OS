...
int main(int argc, char **argv)
{
	...
	if(argc != 7)
	{
		cout <<"usage: ./partitioner.out <path-to-file> <pattern> <search-start-position> <search-end-position> <max-chunk-size> <root-pid>\nprovided arguments:\n";
	...
	pid_t root_pid = atoi(argv[6]);
	
	root_pid=(root_pid==-1)?getpid():root_pid;
	setpgid(0, root_pid);
	...
	if(diff>max_chunk_size){
		...
		if(left_child==0){
			...
			args[6] = strdup(to_string(root_pid).c_str()); 
			args[7] = (char*) nullptr; 
    	...
    	// 	cout << "[" << getpid() << "] left child killed by signal\n";
		// }
    	...
    	// 	cout << "[" << getpid() << "] right child killed by signal\n";
		// }
		...
		if(searcher_child==0){
    		...
    		// 	cout << "[" << getpid() << "] searcher child killed by signal\n";
			// }
